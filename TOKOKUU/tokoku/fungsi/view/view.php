	 ...
	 */ 
	 class view {
		...
		function __construct($db){
			...
			// function pesanan(){
			// 	$sql ="SELECT pesanan.* , barang.id_barang, barang.nama_barang, pesanan.id_pembeli,
			// 			pesanan.nama_pembeli, pesanan.alamat, pesanan.no_hp, nota.id_nota, nota.id_barang, nota.jumlah from pesanan 
			// 		   left join barang on barang.id_barang=nota.id_barang 
			// 		   left join pesanan on pesanan.id_pembeli=nota.id_member 
			// 		   ORDER BY id_pembeli DESC";
			// 	$row = $this-> db -> prepare($sql);
			// 	$row -> execute();
			// 	$hasil = $row -> fetchAll();
			// 	return $hasil;
			// }
			function pesanan_nota(){
				$sql = "SELECT SUM(total) as bayar FROM pesanan";
				$row = $this-> db -> prepare($sql);
				$row -> execute();
				$hasil = $row -> fetchAll();
				return $hasil;
			...
			function pesanan(){
				$sql ="SELECT pesanan.*, nota.jumlah, nota.total, nota.tanggal_input, kategori.nama_kategori
				 FROM pesanan, nota, kategori 
				 WHERE pesanan.id_pembeli = nota.id_nota
				 AND pesanan.id = kategori.id_jenis
				 ORDER BY id DESC";
				$row = $this-> db -> prepare($sql);
				$row -> execute();
				$hasil = $row -> fetchAll();
